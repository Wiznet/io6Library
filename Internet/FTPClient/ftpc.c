...
//uint8_t FTP_destip[4] = {192, 168, 0, 232};	// For FTP client examples; destination network info
uint8_t FTP_destip[16] = {0x20,0x01, 0x02,0xb8,  // For FTP IPv6 client examples; destination network info
						0x00,0x10, 0xff,0xfe,
                        0x31,0x71,0x98,0x05,
                        0x70,0x24,0x4b,0xb1
												};
uint16_t FTP_destport = 25;
...
void ftpc_init()
{
	...
	ftpc.ip_version=sizeof(FTP_destip);
	local_port = 35000;
	strcpy(ftpc.workingdir, "/");
	socket(CTRL_SOCK, Sn_MR_TCPD, ctrl_port, 0x0);
}
...
uint8_t ftpc_run(uint8_t * dbuf)
{
    ...
    switch(tmp=getSn_SR(CTRL_SOCK))
    {
    	case SOCK_ESTABLISHED :
    		...
    		if(gDataSockReady) ftpc_datasock(dbuf);
    	...
    	case SOCK_CLOSED :
    		printf("%d:FTPStart\r\n",CTRL_SOCK);
    		if((ret=socket(CTRL_SOCK, Sn_MR_TCPD, ctrl_port, 0x0)) != CTRL_SOCK){
    	...
    	case SOCK_INIT :
    		printf("%d:Opened\r\n",CTRL_SOCK);
			if((ret = connect(CTRL_SOCK, FTP_destip, FTP_destport, sizeof(FTP_destip))) != SOCK_OK){
   		...
   		case SOCK_CLOSED :
   			if(ftpc.dsock_state == DATASOCK_READY){
   				if(ftpc.dsock_mode == PASSIVE_MODE){
   					...
   					if((ret=socket(DATA_SOCK, Sn_MR_TCPD, local_port, 0x0)) != DATA_SOCK){
   						printf("%d:socket() error:%ld\r\n", DATA_SOCK, ret);
   						close(DATA_SOCK);
   						return ret;
   					}
   		...
   		case SOCK_INIT :
   			...
   			if(ftpc.dsock_mode == ACTIVE_MODE){
   				...
   				printf("%d:Listen ok\r\n",DATA_SOCK);
					...
					if(ftpc.ip_version==IPv6_MODE)
					{
						if((ret = connect(DATA_SOCK, FTP_destip, remote_port, 16)) != SOCK_OK){
							printf("%d:Connect error\r\n", DATA_SOCK);
							return ret;
						}
					}
					...
					{
						if((ret = connect(DATA_SOCK, remote_ip.cVal, remote_port, 4)) != SOCK_OK){
							printf("%d:Connect error\r\n", DATA_SOCK);
							return ret;
						}
					}
